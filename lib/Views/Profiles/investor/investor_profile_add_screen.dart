...
import '../../digilocker/digilocker_verification_screen.dart';
...
class AddInvestorProfileScreen extends StatefulWidget {
  ...
  const AddInvestorProfileScreen({
    Key? key,
    this.initialName,
    this.initialPhone,
    this.initialEmail,
  ...
  @override
  _AddInvestorProfileScreenState createState() => _AddInvestorProfileScreenState();
}
...
class _AddInvestorProfileScreenState extends State<AddInvestorProfileScreen> {
  ...
  String? validateAadhaarName(String? value) {
    ...
    if (RegExp(r'\.\.').hasMatch(value)) {
      return 'Name should not contain consecutive dots';
    }
        ...
        if (mounted) {
          if (response.success) {
            if (response.statusCode == 206) {
              // Navigate to DigiLocker verification
              Navigator.pushReplacement(
                context,
                MaterialPageRoute(
                  builder: (context) => DigiLockerVerificationScreen(
  ...
  Widget _buildInputFields() {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildInputLabel('Investor Name'),
        const Text(
        ...
        Container(
          ...
          child: TextFormField(
            ...
            decoration: InputDecoration(
              ...
              hintText: 'Enter name as per Aadhaar',
              hintStyle: const TextStyle(color: Colors.grey),
              errorStyle: const TextStyle(color: Colors.red),
              suffixIcon: IconButton(
                icon: const Icon(Icons.info_outline, color: Colors.grey),
                onPressed: () => _showAadhaarNameGuidelines(context),
              ),
            ...
            onChanged: (value) {
              ...
              if (value != capitalizedWords) {
                _investorNameController.value = TextEditingValue(
  ...
  bool _formHasData() {
    return _investorNameController.text.isNotEmpty ||
  ...
  void _showAadhaarNameGuidelines(BuildContext context) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Name Guidelines'),
          content: SingleChildScrollView(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              mainAxisSize: MainAxisSize.min,
              children: const [
                Text('Please ensure your name matches exactly with your Aadhaar card:'),
                SizedBox(height: 16),
                Text('• Use your full name as shown on Aadhaar'),
                Text('• Include initials if present on Aadhaar'),
                Text('• Maintain the same spelling'),
                Text('• Include dots (.) if present in initials'),
                Text('• Maintain the same order of names'),
                SizedBox(height: 16),
                Text('Examples:', style: TextStyle(fontWeight: FontWeight.bold)),
                Text('✓ "John K. Smith"'),
                Text('✓ "M. K. Gandhi"'),
                Text('✓ "Mary Jane Peter"'),
                SizedBox(height: 16),
                Text('Note: Mismatched names may cause verification issues later.'),
              ],
            ),
          ),
          shape: RoundedRectangleBorder(
