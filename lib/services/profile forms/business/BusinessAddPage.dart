...
class BusinessAddPage {
  ...
  static Future<bool?> businessAddPage({
    ...
    try {
      ...
      // Create multipart request
      var request = http.MultipartRequest('POST', Uri.parse(ApiList.businessAddPage!));
      ...
      // Add files to the request (only if they are not null)
      if (image1 != null) {
        request.files.add(await http.MultipartFile.fromPath('image1', image1.path, filename: basename(image1.path)));
      }
      if (image2 != null) {
        request.files.add(await http.MultipartFile.fromPath('image2', image2.path, filename: basename(image2.path)));
      }
      if (image3 != null) {
        request.files.add(await http.MultipartFile.fromPath('image3', image3.path, filename: basename(image3.path)));
      }
      if (image4 != null) {
        request.files.add(await http.MultipartFile.fromPath('image4', image4.path, filename: basename(image4.path)));
      }
      if (doc1 != null) {
        request.files.add(await http.MultipartFile.fromPath('doc1', doc1.path, filename: basename(doc1.path)));
      }
      if (proof1 != null) {
        request.files.add(await http.MultipartFile.fromPath('proof1', proof1.path, filename: basename(proof1.path)));
      }
      ...
      // Handle the response
      if (response.statusCode == 201) {
        log('Business added successfully!');
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        log('Failed to add business: ${response.statusCode}');
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Failed to add business: ${response.statusCode}'),
            duration: Duration(seconds: 3),
          ),
        );
        return false;
